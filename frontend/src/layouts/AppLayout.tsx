  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-4 py-5 border-b"
        ...
        {!collapsed && (
          <div>
            <div className="font-syne font-bold text-sm" style={{ color: "hsl(var(--foreground))" }}>FieldScope</div>
            <div className="text-[10px]" style={{ color: "hsl(var(--muted-foreground))" }}>Business Intelligence</div>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-5">
        {navSections.map((section) => (
          <div key={section.label}>
            {!collapsed && (
              <div className="px-2 mb-1.5 text-[10px] font-bold tracking-widest uppercase"
            ...
            <div className="space-y-0.5">
              {section.items.map((item) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-150 group relative ${isActive ? "nav-active" : "nav-inactive"}`
                  }
                  style={({ isActive }) => ({
                    ...
                    <>
                      ...
                      {!collapsed && (
                        <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded font-bold"
                          style={{ background: `${phaseColors[item.phase]}18`, color: phaseColors[item.phase] }}>
                          P{item.phase}
                        </span>
                      )}
                    </>
      ...
      <div className="p-3 border-t space-y-1" style={{ borderColor: "hsl(var(--sidebar-border))" }}>
        <button className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all"
          style={{ color: "hsl(var(--muted-foreground))" }} onClick={() => {}}>
          <Settings size={15} />
          {!collapsed && <span>Settings</span>}
        </button>
        <button className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all"
          style={{ color: "hsl(var(--muted-foreground))" }} onClick={() => navigate("/")}>
          <LogOut size={15} />
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: "hsl(var(--background))" }}>
      ...
      {mobileOpen && (
        <>
          <div className="fixed inset-0 z-40 md:hidden"
            ...
            onClick={() => setMobileOpen(false)} />
          <aside className="fixed left-0 top-0 bottom-0 z-50 w-64 flex flex-col md:hidden border-r"
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-1 max-w-xs px-3 py-2 rounded-lg border text-sm"
            style={{ background: "hsl(var(--navy-700))", borderColor: "hsl(var(--border))", color: "hsl(var(--muted-foreground))" }}>
            <Search size={14} />
          ...
          <div className="ml-auto flex items-center gap-3">
            <button className="relative p-2 rounded-lg border transition-all"
              style={{ background: "hsl(var(--navy-700))", borderColor: "hsl(var(--border))", color: "hsl(var(--muted-foreground))" }}>
              <Bell size={16} />
              <span className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full animate-pulse-glow"
                style={{ background: "hsl(var(--cyan))" }} />
            </button>

            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm cursor-pointer"
